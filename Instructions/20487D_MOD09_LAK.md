...
5.  Line 13 create new instance of **HttpClient** which allows us to create HTTP requests.
    ```cs
        HttpClient client = new HttpClient();
    ```
6.  Line 15 create an HTTP **POST** call by using **HttpClient** and passing **username** and **password** to register a new user.
    ```cs
    ...
    ```
7.  Line 16 extract **token** by using the return result of the last call.
    ```cs
        string token = await postRes.Content.ReadAsStringAsync();
    ```
8.  Line 17 store the token in the header for the next authentication requests.
    ```cs
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ``
9.  Line 18 create an HTTP **GET** request with the token store in the header.
    ```cs
        response = await client.GetAsync("https://localhost:5001/api/values");
    ``
10. Line 19 read the content by using the return result of the last call.
    ```cs
...
15. Select the **Sign-up** attributes. Choose attributes you want to collect from the consumer during sign-up. For example, select **Country/Region**, **Display Name**, and **Postal Code**. Click **Create**.
...
11.  Add the following configuration and replace *[TenantName]* and *{YourInitials}* with the corresponding values.
    ...
    ```
12. Open the **ValuesController.cs** file.
...
4. Open the browser and navigate to the following URL:
    ...
    ```
5. Click **Login**, and the sign in as an Azure Active DIrectory B2C user.
6. To see the result, click **Request**.
