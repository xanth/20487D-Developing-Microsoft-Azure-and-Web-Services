...
3. To create a new **ApplicationDbContext** class, rigth-click **Data** folder, select **New File** and name it **ApplicationDbContext.cs**
...
6.  To add **class declaration**, inside the **namespace** brackets, enter the following code:
    ...
    ```
7.  To add **constructor** to the class, inside the **class** brackets, enter the following code:
    ```cs
...
11. To add **class declaration**, inside the **namespace** brackets, enter the following code:
    ...
    ```
12. To add **Initialize** method to the **class** brackets, enter the following code:
    ```cs
    ...
    ``
13. To add **Seed** method to the **class** brackets, enter the following code:
    ```cs
...
2.  Add the following **using** to the top of the page:
    ```cs
        ...
        using System.Text;
    ```
3.  Inside **ConfigureServices** method before **services.AddMvc() ...** line, add the following code:
    ```cs
    ...
    ```
4.  Locate **Configure** method and before **app.UseHttpsRedirection();** call, add the following code:
    ```cs
        app.UseAuthentication();
    ```
5. Open **appsettings.json** file.
6. To add a **connection string** and **JWT** configuration paste the brackets following code inside the :
   ```json
    "ConnectionStrings": {
...
2.  To create a new **LoginDto** class, rigth-click **Models** folder, select **New File** and name it **LoginDto.cs**
3.  In the **LoginDto** class, add the following **using** statements:
    ```cs
...
6.  In **LoginDto** class, add the following properties:
    ...
    ```
7.  To create a new **RegisterDto** class, rigth-click **Models** folder, select **New File** and name it **RegisterDto.cs**
...
18. In the **UserController** class, **Declare GenerateJwtToken** method with **email** and **user** parameters:
    ```cs
    ...
    ```
19. To create new token, inside **GenerateJwtToken** method paste the following code:
    ```cs
    ...
    ```
20. To add **Register** action, inside the **class** brackets, enter the following code:
    ```cs
        [HttpPost("register")]
    ...
    ```
21. To create **new IdentityUser** paste the following code:
    ```cs
    ...
    ```
22. To create new **user**, paste the following code:
    ```cs
        IdentityResult result = await _userManager.CreateAsync(user, model.Password);
    ```
23. To verify the save result succeeded and return the token, paste the following code
    ```cs
        if (result.Succeeded)
        {
    ...
    ```
24. Add **Login** action, inside the **class** brackets, enter the following code:
    ```cs
        [HttpPost("login")]
    ...
    ```
25. In the **Login** method, call the **PasswordSignInAsync** and pass **Username** and **Password** then store the return result:
    ```cs
        var result = await _signInManager.PasswordSignInAsync(model.Username, model.Password, false, false);
    ```
26. To verify the login result succeeded and return the token, paste the following code:
    ```cs
        if (result.Succeeded)
        {
...
1. Expand **Controllers** folder and double-click on **ValuesController.cs** file.
...
5.  Line 13 create new instance of **HttpClient** which allow us to create http requests.
    ...
    ```
6.  Line 15 create http **POST** call using the **HttpClient** and passing **username** and **password** to register new user
    ```cs
        HttpResponseMessage postRes = await client.PostAsync("https://localhost:5001/api/user/register", new StringContent("{ \"Username\": \"azure@azure.com\", \"Password\": \"Azure123!!\" }", Encoding.UTF8, "application/json"));
    ```
7.  Line 16 extract **token** using the return result of the last call
    ```cs
        string token = await postRes.Content.ReadAsStringAsync();
    ```
8.  Line 17 store the token at the header for next authenticat requests
    ```cs
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ``
9.  Line 18 create http **GET** request with the token store in the header
    ```cs
        response = await client.GetAsync("https://localhost:5001/api/values");
    ``
10. Line 19 read the content using the return result of last call
    ```cs
...
3.  To run the project, run the following command:
    ```bash
        dotnet run
    ```
4.  Open new instance of Command Prompt window.
4.  To change the directory to the **IdentityClient** project, run the following command:
    ```bash
        cd [Repository Root]\Allfiles\Mod09\LabFiles\Lab1\Starter\IdentityClient
    ```
5.  To run the project, run the following command:
    ```bash
        dotnet run
    ```
6.  Verify that **[value1, value2]** return back to your Command Prompt.
...
5.  Add the following **using** to the top of the page:
    ```cs
        ...
        using Microsoft.AspNetCore.Authentication.JwtBearer;
    ```
6.  Inside **ConfigureServices** method before **services.AddMvc()** line, add the following code:
    ```cs
        services.AddAuthentication(options =>
        {
            options.DefaultScheme = JwtBearerDefaults.AuthenticationScheme;
...
4. Open browser and navigate to the following url:
    ...
    ```
5. Click the **Login** button and login as Azure Active DIrectory B2C user
6. Click the **Request** button you should see the result
