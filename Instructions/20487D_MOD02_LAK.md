...
5. To add a private field for **DbContextOptions**, paste the following code:
   ...
   ```
6. To add a new constructor that initialize the **options**, paste the following code:
   ```cs
   ...
   ``` 
7. To add a new constructor with **options** parameter, paste the following code:
   ```cs
...
1. Switch to the Command Prompt window.
2. Change the directory to **DAL.Test**, at the command prompt, run the following command:
   ```bash
    cd [Repository Root]\Allfiles\Mod02\LabFiles\Lab1\Starter\DAL.Test
   ```
