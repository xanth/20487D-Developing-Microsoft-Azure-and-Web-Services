...
5. To add a private field for **DbContextOptions**, paste the following code:
   ```cs
    private DbContextOptions<MyDbContext> _options;
   ```
6. To add a new constructor with an optional parameter, paste the following code:
   ```cs
   ...
   ```
7. To add a new booking entity to **Database**, inside **Class**, paste the following method:
   ```cs
...
10. Right-click **dbo.Bookings**, and then select **Select Top 1000**.
11. Check that both the bookings are saved in the database.

#### Task 3: Replace the SQL Server provider with SQLite

1. Switch to the Command Prompt window.
2. Change the directory to **DAL**, at the command prompt, run the following command:
   ```bash
    cd [Repository Root]\Allfiles\Mod02\LabFiles\Lab1\Starter\DAL
   ```
