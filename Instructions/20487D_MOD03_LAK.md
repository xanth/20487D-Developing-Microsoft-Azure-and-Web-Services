...
### Exercise 1: Use WebListener

#### Task 1: Add the WebListener NuGet and add a new option to the list of commands

1.  Open **Command Line**.
2. At the **Command Line** paste the following command to change directory to **BlueYonder.Hotels.Service** project:
    ```bash
    cd [Repository Root]\Allfiles\Mod03\LabFiles\Lab2\Starter\BlueYonder.Hotels.Service
    ```
3. Run the following command to install **WebListener** package:
   ```bash
    dotnet add package Microsoft.AspNetCore.Server.WebListener --version=1.1.4
   ```
4. Run the following command to change directory to **Starter** project:
   ```bash
    cd ..
   ```
5. Restore all dependencies and tools of a project use the following command in the **Command Line**:
    ```bash
    dotnet restore
    ```
6. Run the following command to open the project in **VSCode**: 
    ...
    code .
    ```
7. In **VSCode** explorer panel, expand **BlueYonder.Hotels.Service** folder and double-click on **Program.cs**.
8. Paste the following code inside the **Main** method to get arguments from the **Command Line** :
   ```cs
   ...
   ```
9.  Paste the following code to create a **WebHostBuilder**:
    ```cs
     IWebHostBuilder builder = CreateWebHostBuilder(args);
    ```  
10. Paste the following code to check if argument has **weblistener** mode:
    ```cs
    ...
    ```
11. Paste the following code to add a default mode using **IIS express**
    ```cs
     else
     {
...
12. Paste the following code to run the **Builder**:
    ```cs
    builder.Build().Run();
    ```

#### Task 2: Run the project using WebListener and retest with a browser
...
2. Run the following command to change directory to **BlueYonder.Hotels.Service**
   ```bash
    cd [Repository Root]\Allfiles\Mod03\LabFiles\Lab2\Starter\BlueYonder.Hotels.Service
   ```
3. Run the following command to run the project with **WebListener** mode:
   ```bash
    dotnet run --mode=weblistener
   ```
4. Open browser and navigate to the following **URL**:
   ```url
