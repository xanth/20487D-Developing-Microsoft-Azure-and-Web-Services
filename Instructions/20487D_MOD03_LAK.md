...
1. Switch to the Command Prompt window.
2. To run the project with the **Kestrel** mode, run the following command:
   ```bash
    dotnet run --mode=Kestrel
   ```
3. Open a new command prompt.
4. To change the directory to **BlueYonder.Hotels.Client**, run the following command.
   ```bash
    cd [Repository Root]\Allfiles\Mod03\LabFiles\Lab2\Starter\BlueYonder.Hotels.Client
   ```
5. To run the **BlueYonder.Hotels.Client** project, run the following command:
   ```bash
    dotnet run
   ```
6. At the command prompt, verify that the **POST** and **PUT** requests succeeded.
7. Close all windows.
