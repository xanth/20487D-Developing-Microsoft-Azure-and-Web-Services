...
1. Open PerfView.
2. In the top menu bar, click **Collect**.
...
6. View the exception call stack, find the line with the text **Blueyonder.Service.Controllers.FlightsControllers.Get(int32)** this is the service that throw the exception.
7. On the top bar, in the **GroupPats** list, select **[no grouping]**.
8. Expand **Blueyonder.Service.Repository.FlightsRepository.HashFlightCode(int32)**.
9. View the exception call stack to see which method in the service throws the exception.
...
3. To install **software-properties-common**, run the following command:
   ...
   ```
4. To add **PPA repository**, run the following command:
   ```bash
...
1. To archive the trace folder, run the following command:
   ...
   ```
2. To exit bash, run the following command:
   ```bash
   exit
   ```
3. To copy the archive to the local file system, run the following command:
   ```bash
   docker cp myapp:app/trace.zip .
   ```
4.  To kill the running container, run the following command:
   ```bash
   docker kill myapp
   ```
...
3. On the left menu, in the **Configure** section, click **Performance test**.
4. On the top menu bar, click **Set Organization**, and then add the following information:
    - In **Organization Settings**, click **Or Create New**, and then type **blueyondervsts***{YourInitials}*.
...
1. Switch to VSTS.
2. On the top menu bar, place the mouse pointer over **Test Plans**, and then click **Load test**.
...
3. On the right-hand side, view the info about the request.
